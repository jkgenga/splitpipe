...
insert into hibernate_sequence values ( 1 )
insert into hibernate_sequence values ( 1 )
create table LEAGUE (LEAGUE_ID bigint not null, NAME varchar(255) not null, SEASON_ID bigint, primary key (LEAGUE_ID))
create table LEAGUE_RESULT (LEAGUE_RESULT_ID bigint not null, DRAWS integer, GAMES integer, GOALS integer, GOALS_GOT integer, LOOSES integer, POINTS integer, TEAM_ID tinyblob, WINS integer, LEAGUE_ID bigint not null, primary key (LEAGUE_RESULT_ID))
...
create table MATCHES (MATCH_ID bigint not null, MATCH_DATE date, GOALS_GUEST integer, GOALS_HOME integer, RESULT varchar(255), TINGLER time, MATCH_DAY_ID bigint, TEAM_GUEST_ID bigint, TEAM_HOME_ID bigint, primary key (MATCH_ID))
...
create table SEASONS (SEASON_ID bigint not null, ACTUAL_GAME_DAY integer not null, BEGIN_YEAR integer not null, NAME varchar(255) not null, primary key (SEASON_ID))
...
create table USERS (USER_ID bigint not null, GIVEN_NAME varchar(255), PASSWORD varchar(255), SURNAME varchar(255), USER_NAME varchar(255) not null, primary key (USER_ID))
alter table LEAGUE add constraint UKo2kbx7nomatkf3rtebmask919 unique (NAME, SEASON_ID)
